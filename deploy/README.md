...
- A subdomain to point at the machine, e.g. `api.ammalutex.com`
...
Add an `A` record for `api.ammalutex.com` pointing to the VM's public IP.
...
curl https://api.ammalutex.com/health     # {"status":"healthy"}
```

## 5. Point the frontend at it

Vercel → project → Settings → Environment Variables →
`NEXT_PUBLIC_API_URL = https://api.ammalutex.com` → redeploy.
...
`dig api.ammalutex.com` returns the VM's IP, then `docker compose restart caddy`.
