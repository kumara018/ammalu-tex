      ...
      <div className="max-w-[54rem]">
        ...
        <Row
          ...
          note={d.payments?.configured && !d.payments.webhook
            ...
            : undefined}
        />
        {/* CONFIGURED IS NOT DELIVERED, and this row is why that distinction
            earns its keep. The SMTP host was hardcoded to Gmail while the
            shop's mailbox is elsewhere, so credentials were present, this row
            would have been green, and not one message ever left. */}
        <Row
          label="Email"
          tone={
        ...
        />
        {/* A TOKEN BEING PRESENT IS NOT A TOKEN BEING ACCEPTED. This shop has
            one set and Delhivery answers 401 Unauthorized; the row used to go
            green on presence alone, the same bug the email row had. */}
        <Row
          label="Courier"
          tone={
          ...
          note={d.courier?.configured && !d.courier.return_address
            ...
            : undefined}
        />
        {/* A sender being set is not a message arriving - the shared Twilio
            sandbox reaches only phones that have joined it. */}
        <Row
          label="WhatsApp"
          tone={
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={(form as any).fit || ''} onChange={e => setForm(f => ({ ...f, fit: e.target.value } as any))}
                    placeholder="Regular Fit, Slim Fit, Relaxed..." className="input-field" />
                </div>
                <div>
              ...
              <div>
                ...
                <textarea value={(form as any).care_instructions || ''} onChange={e => setForm(f => ({ ...f, care_instructions: e.target.value } as any))}
                  rows={2} placeholder="Machine wash cold. Do not bleach. Tumble dry low. Iron on medium heat."
