...
const FALLBACK = 'https://ammalu-tex.onrender.com';
...
describe('the origin the code calls is the origin the policy allows', () => {
  ...
  const cases: Array<[name: string, value: string | undefined]> = [
    ['unset', undefined],
    ['the new backend', 'https://api.ammalutex.com'],
    ['a pasted URL with a trailing slash and a path', 'https://api.ammalutex.com/api/'],
    ['whitespace around it', '  https://api.ammalutex.com  '],
...
describe('what the variable actually does', () => {
  it('moves the backend when it is a real origin — the whole point of it', async () => {
    const c = await configFor('https://api.ammalutex.com');
    expect(c.calls).toBe('https://api.ammalutex.com');
    expect(c.baked).toBe('https://api.ammalutex.com');
  });
  ...
  it('normalises a path, a trailing slash and stray whitespace away', async () => {
    ...
    const c = await configFor('  https://api.ammalutex.com/api/  ');
    expect(c.calls).toBe('https://api.ammalutex.com');
    expect(c.baked).toBe('https://api.ammalutex.com');
  });
