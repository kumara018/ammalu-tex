 ...
 * empty it falls back to featured products so the space is never blank.
 *
 * The fallback carries this shop today, and unusually well: its product
 * photographs are full-length lifestyle shots on the model, which is exactly
 * what the opening wants. Only four of nineteen pieces are photographed, so
 * the rotation is four deep until more arrive.
 *
 ...
 *
 * It also happens to be the better test. Measured across THIS shop's library,
 * which is only four photographed pieces but all of one kind:
 *
 *     Floral Printed Tops       600 x 900   0.667   a garment, on the model
 *     Traditional Chudithar     608 x 760   0.800   a garment, on the model
 *
 * Both sit inside the band. The two frames that would fail it are the ones
 * this test exists for: a collage is near 1.000, and a cropped close-up runs
 * well under 0.62. Neither is in the library today, and the check costs
 * nothing while the shop keeps adding photographs.
 */
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            'linear-gradient(90deg, rgba(250,246,243,0.97) 0%, rgba(250,246,243,0.93) 30%, rgba(250,246,243,0.55) 58%, rgba(250,246,243,0.15) 100%)',
        }}
      />
    </div>
  );
