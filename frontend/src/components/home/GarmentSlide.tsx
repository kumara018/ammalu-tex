 ...
 * band.
 *
 * MEASURED, WHICH IS WHY IT CHANGED. The band renders 1434x361 on a 1440px
 * screen — 3.97:1, almost exactly a LinkedIn cover. The photographs going
 * into it are 1440x2162, which is 0.67:1. A 4:1 frame can therefore show
 * about a sixth of a portrait photograph's height, and what a customer saw
...
/** The ground the wash is made of — the shop's paper. */
const WASH = '250,246,243';
...
export default function GarmentSlide({ products }: { products: Product[] }) {
   ...
   * lane runs out and the ground shows through before it wraps. This shop has
   * four photographed pieces today, four cards is barely a thousand pixels,
   * and a desktop band is 1900. So the set is repeated until one half covers
      ...
      >
        {lane.map((src, i) => (
          ...
          <div key={`${src}-${i}`} className="h-full shrink-0 pr-3">
            <div className="h-full bg-paper-shade" style={{ aspectRatio: '3 / 4' }}>
              <img
