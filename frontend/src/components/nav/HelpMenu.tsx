...
const ROWS = [
  ...
  {
    label: 'Will it fit?',
    detail: 'S to XXXL, measured flat',
  ...
  return (
    <div ref={wrap} className="relative">
      <button
        ...
        aria-expanded={open}
        aria-haspopup="menu"
        className={`text-rule uppercase transition-colors duration-300 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-thread ${
          open ? 'text-thread' : 'text-graphite-muted hover:text-thread'
        }`}
      >
        Help
      </button>
      ...
      {open && (
        <div
          ...
          className="absolute right-0 top-7 z-50 w-[min(92vw,21rem)] border border-paper-edge bg-paper-bright py-1"
        >
          {ROWS.map((r) => (
            <Link
              key={r.href}
              ...
              className="block px-4 py-2.5 transition-colors duration-200 hover:bg-paper-shade focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-thread"
            >
              <span className="block text-sm text-graphite">{r.label}</span>
              <span className="block text-xs text-graphite-faint">{r.detail}</span>
            </Link>
          ))}
        </div>
      )}
