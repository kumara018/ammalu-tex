    ...
    <div ref={wrap} className="relative">
      ...
      {open && (
        <div
          ...
          className="absolute right-0 top-7 z-50 w-[min(92vw,20rem)] border border-paper-edge bg-paper-bright py-1"
        >
          {rows.map((r) => (
            <a
              key={r.label}
              ...
              className="flex gap-3 px-4 py-2.5 transition-colors duration-200 hover:bg-paper-shade focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-thread"
            >
              <Icon kind={r.icon} />
              <span className="min-w-0">
                <span className="block text-sm text-graphite">{r.label}</span>
                <span className="block truncate text-xs text-graphite-faint">{r.detail}</span>
              </span>
            </a>
          ))}
        </div>
