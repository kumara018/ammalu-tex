...
export function EmptyState({
  ...
  return (
    <div className="border-t border-paper-edge py-[10vh]">
      {eyebrow && <p className="mb-6 text-rule uppercase text-graphite-faint">{eyebrow}</p>}
      <h2 className="max-w-[22ch] font-display text-band font-light text-graphite">{title}</h2>
      {body && <p className="mt-6 max-w-[48ch] text-lede text-graphite-muted">{body}</p>}
...
export function ErrorState({
  ...
  return (
    <div role="alert" className="border-t border-paper-edge py-[10vh]">
      <p className="mb-6 text-rule uppercase text-thread">Something went wrong</p>
      <h2 className="max-w-[22ch] font-display text-band font-light text-graphite">{title}</h2>
      <p className="mt-6 max-w-[48ch] text-lede text-graphite-muted">{body}</p>
...
export function SkeletonLine({ w = 'w-full', h = 'h-3' }: { w?: string; h?: string }) {
  return <div className={`${h} ${w} rounded-full bg-graphite/[0.07]`} />;
}

export function SkeletonBlock({ className = '' }: { className?: string }) {
  return <div className={`rounded-sm bg-graphite/[0.05] ${className}`} />;
}
