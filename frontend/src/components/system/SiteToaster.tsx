      ...
      {(t: Toast) => (
        ...
        >
          {({ message }) => {
            ...
            return (
              <div
                className={`flex w-full items-stretch border bg-paper-bright shadow-[0_6px_24px_-12px_rgba(51,39,34,0.35)] ${
                  bad ? 'border-critical/50' : 'border-paper-edge'
                }`}
              >
                {/* The rule, not an icon. Colour carries the status. */}
                <span
                  aria-hidden="true"
                  className={`w-[3px] shrink-0 ${bad ? 'bg-critical' : 'bg-thread-deep'}`}
                />
                <div className="min-w-0 px-4 py-3">
                  <span
                    className={`block text-rule uppercase ${
                      bad ? 'text-critical' : 'text-thread-deep'
                    }`}
                  >
