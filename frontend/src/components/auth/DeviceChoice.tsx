  ...
  return (
    <div>
      <h2
        ref={headingRef}
        tabIndex={-1}
        className="font-display text-xl font-light text-graphite focus:outline-none"
      >
      ...
      {error && (
        <p role="alert" className="mt-5 text-xs text-thread">
          {error}
        </p>
      )}
      ...
      <ul className="mt-7">
        {sessions.map((s) => {
          ...
          return (
            <li key={s.id} className="border-t border-paper-edge py-5 first:border-t-0 first:pt-0">
              <div className="flex items-start justify-between gap-5">
                <div className="min-w-0">
                  <p className="text-rule uppercase text-graphite-faint">
                    {TYPE_LABEL[s.device_type ?? ''] ?? 'Device'}
                  </p>
                  <p className="mt-1.5 text-graphite">{label}</p>
                  {s.location && <p className="mt-1 text-xs text-graphite-faint">{s.location}</p>}
                  <p
                    className="mt-1 text-xs text-graphite-faint"
