...
class Category(Base):
    ...
    #
    # The columns this application filters and orders by had no index at all —
    # the `index=True` scattered through this file is almost entirely primary
    # keys, which were already indexed. Measured on the sister shop with the
    # same schema, eight of ten hot query shapes were full table SCANs, most
    # with a temporary B-tree sort on top.
    #
    # Composite and column-ordered on purpose: an index on `user_id` alone
    # still leaves the database sorting the matched rows by hand, and that
    # sort grows with one customer's order history — which makes it worst for
    # the customer who buys most.
...
class Product(Base):
    ...
    fit                = Column(String(100), nullable=True)   # e.g. "Regular Fit", "Slim Fit"
